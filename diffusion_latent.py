...
import sys
sys.path.append('../iznik/Iznik-Pottery--Imagine')

from models import orig_unet
from models.ddpm import DDPM as TileDDPM

from models_asyrp.guided_diffusion.script_util import guided_Diffusion
...
from models_asyrp.diffusion import DDPM
from models_asyrp.improved_ddpm.script_util import i_DDPM
from utils_asyrp.diffusion_utils import get_beta_schedule, denoising_step
from utils_asyrp.text_dic import SRC_TRG_TXT_DIC
...
class Asyrp(object):
    ...
    def load_pretrained_model(self):
        ...
        elif self.config.data.dataset in ["FFHQ", "AFHQ", "IMAGENET", "MetFACE", "TILES"]:
        ...
        elif self.config.data.dataset == "TILES":
            unet = orig_unet.UNet(64, channels=3, dim_mults=(1, 2, 4,))
            model = TileDDPM(unet, 1000, beta_start=0.0001, beta_end=0.02)
            if self.args.model_path:
                init_ckpt_orig = torch.load(self.args.model_path)
                if 'state_dict' in init_ckpt_orig:
                    init_ckpt_orig = init_ckpt_orig['state_dict']
                init_ckpt = dict()
                for k, v in init_ckpt_orig.items():
                    if k[:4] == 'ddpm':
                        continue
                    new_key = 'network' + k[5:]
                    init_ckpt[new_key] = v
            else:
                raise ValueError(f'Please specify --model_path for {self.config.data.dataset}.')
            self.learn_sigma = False
            print("My Tile DDPM loaded.")
    ...
    def run_training(self):
        ...
        if self.args.train_delta_block:
            ...
            for i in range(self.args.get_h_num):
                if hasattr(model.module, 'network'):
                    # is other implementation of DDPM
                    get_h = getattr(model.module.network, f"layer_{i}")
                else:
    ...
    def save_image(self, model, x_lat_tensor, seq_inv, seq_inv_next,
        ...
        tvu.save_image(grid, os.path.join(folder_dir, f'{file_name}_ngen{self.args.n_train_step}.png'), normalize=True)
            ...
            if os.path.exists(pairs_path) and not self.args.re_precompute:
                ...
                if save_imgs:
                    for step, (x0, x_id, x_lat) in enumerate(img_lat_pairs_dic[mode]):
                        print(f"saving image to {os.path.join(self.args.image_folder, f'{mode}_{step}_0_orig.png')}")
                        print(f"saving image to {os.path.join(self.args.image_folder, f'{mode}_{step}_1_rec_ninv{self.args.n_inv_step}.png')}")
                        print()
    ...
    def compute_lpips_distance(self):
        ...
        with open(os.path.join("../Asyrp_official/utils_asyrp", f"{(self.args.config).split('.')[0]}_LPIPS_distance_x.tsv"), "w") as f:
            f.write(result_x_tsv)
        with open(os.path.join("../Asyrp_official/utils_asyrp", f"{(self.args.config).split('.')[0]}_LPIPS_distance_x_std.tsv"), "w") as f:
            f.write(result_x_std_tsv)
        with open(os.path.join("../Asyrp_official/utils_asyrp", f"{(self.args.config).split('.')[0]}_LPIPS_distance_x0_t.tsv"), "w") as f:
            f.write(result_x0_tsv)
        with open(os.path.join("../Asyrp_official/utils_asyrp", f"{(self.args.config).split('.')[0]}_LPIPS_distance_x0_t_std.tsv"), "w") as f:
    ...
    def set_t_edit_t_addnoise(self, LPIPS_th=0.33, LPIPS_addnoise_th=0.1, return_clip_loss=False):
        ...
        LPIPS_file_path = os.path.join("./precomputed_lpips", LPIPS_file_name)
        ...
        if self.args.user_defined_t_addnoise:
            ...
            if self.args.add_noise_from_xt:
                ...
                LPIPS_file_path = os.path.join("../Asyrp_official/utils_asyrp", LPIPS_file_name)
                if not os.path.exists(LPIPS_file_path):
