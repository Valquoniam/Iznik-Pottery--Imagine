...
def load_network_pkl(f):
    ...
    # Validate contents
    assert isinstance(data["G"], torch.nn.Module) #type: ignore
    assert isinstance(data["D"], torch.nn.Module) #type: ignore
    assert isinstance(data["Gs"], torch.nn.Module) #type: ignore
    return data 
...
def convert_tf_generator(tf_G):
    ...
    kwargs = dnnlib.EasyDict(
        ...
        k                       = kwarg("components_num",       1) + int(tf_G.static_kwargs.get("transformer", False)), #type: ignore
        ...
        synthesis_kwargs = dnnlib.EasyDict(
            ...
            channel_base        = kwarg(["fmap_base", "channel_base"],   16 << 10) * 2, #type: ignore
