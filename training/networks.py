...
def get_weight(shape, gain = 1, use_wscale = True, lrmul = 1, channels_last = False):
    ...
    w = torch.nn.Parameter(format_memory(w, channels_last))#type: ignore
...
def get_bias(num_channels, bias_init = 0, lrmul = 1):
    b = torch.nn.Parameter(torch.full([num_channels], np.float32(bias_init)))#type: ignore
    return b, lrmul
...
class BiasActLayer(torch.nn.Module):#type: ignore
...
@persistence.persistent_class
class FullyConnectedLayer(torch.nn.Module):#type: ignore
    def __init__(self, in_channels, out_channels, bias = True, act = "linear", gain = 1, lrmul = 1, bias_init = 0):
        ...
        self.bias,   self.b_gain = get_bias(out_channels, bias_init, lrmul) if bias else None#type: ignore
        self.act = act
    ...
    def forward(self, x, _x = None): # _x is added to the signature for backward-compatibility and isn't used
        ...
        if self.act == "linear" and b is not None:
            x = torch.addmm(b.unsqueeze(0), x, w.t())#type: ignore
        else:
            x = x.matmul(w.t())#type: ignore
            x = bias_act.bias_act(x, b, act = self.act)
        return x

# Resent Layer
@persistence.persistent_class
class ResnetLayer(torch.nn.Module):#type: ignore
    ...
    def forward(self, x, _x):
        ...
        x = torch.nn.functional.leaky_relu(x + _x, negative_slope = 0.2) #type: ignore
        return x
    ...
    def forward(self, x, pos = None, mask = None):
        ...
        for idx in range(self.layers_num):
            ...
            if self.sa:
                ...
                x = sa(from_tensor = x, to_tensor = x, from_pos = pos, to_pos = pos, att_mask = mask.unsqueeze(1))[0]#type: ignore
...
@persistence.persistent_class
class Conv2dLayer(torch.nn.Module):#type: ignore
    def __init__(self,
...
def modulated_conv2d(
    ...
    if demodulate:
        d = (w.square().sum(dim = [2,3,4]) + 1e-8).rsqrt() # [NO]#type: ignore
    if demodulate and fused_modconv:
        w = w * d.reshape(batch_size, -1, 1, 1, 1) # [NOIkk]#type: ignore

    if fused_modconv:
        # Type conversations
        w = w.to(x.dtype)#type: ignore
        ...
        if demodulate and noise is not None:
            x = fma.fma(x, d.reshape(batch_size, -1, 1, 1), noise)#type: ignore
        elif demodulate:
            x = x * d.reshape(batch_size, -1, 1, 1)#type: ignore
...
def get_linear_encoding(size, dim, num, rng = 1.0):
    ...
    embs = torch.nn.Parameter(torch.rand([num, int(dim / num)]))#type: ignore
...
def get_positional_encoding(
    ...
    if pos_type == "sinus":
        ...
        emb = torch.nn.Parameter(initializer([res, res, pos_dim]))#type: ignore
        params = [emb]
    else: # pos_type == "trainable"
        xemb = torch.nn.Parameter(initializer([res, int(pos_dim / 2)]))#type: ignore
        yemb = xemb if shared else torch.nn.Parameter(initializer(res, int(pos_dim / 2)))#type: ignore
        params = [xemb, yemb]
        xemb = xemb.unsqueeze(0).tile([res, 1, 1])
        yemb = yemb.unsqueeze(1).tile([1, res, 1])
        emb = torch.cat([xemb, yemb], dim = -1)

    emb = misc.crop_tensor(emb, crop_ratio)#type: ignore
    return emb, params
...
def get_embeddings(size, dim, init = "uniform", name = None):
    ...
    emb = torch.nn.Parameter(initializer([size, dim]))#type: ignore
    return emb
...
def compute_probs(scores, dp_func):
    ...
    probs = torch.nn.functional.softmax(scores, dim = -1) # [B, N, F, T]#type: ignore
...
@persistence.persistent_class
class GateAttention(torch.nn.Module):#type: ignore
...
@persistence.persistent_class
class TransformerLayer(torch.nn.Module):#type: ignore
    def __init__(self,
        ...
        self.att_dp = torch.nn.Dropout(p = attention_dropout / 2) #type: ignore
        ...
        # Centroids
        if self.kmeans:
            self.att_weight = torch.nn.Parameter(torch.ones(num_heads, 1, self.centroid_dim))#type: ignore
            if self.parametric:
                self.centroids = torch.nn.Parameter(torch.randn([1, num_heads, to_len, self.centroid_dim]))#type: ignore
            else:
    ...
    def process_input(self, t, t_pos, name):
        ...
        if t_pos is not None:
            ...
            t_pos = t_pos.tile([batch_size, 1])#type: ignore
    ...
    def compute_centroids(self, _queries, queries, to_from, hw_shape):
        ...
        return from_elements, to_centroids#type: ignore
        ...
        for i in range(self.kmeans_iters):
            if self.kmeans:
                if i > 0:
                    ...
                    # 2. 'from' elements (queries)
                    # Compute the 'to' respective centroids
                    to_centroids = to_from.matmul(from_elements)#type: ignore

                # Compute attention scores based on dot products between
                # 'from' queries and the 'to' centroids.
                att_scores = (from_elements * self.att_weight).matmul(to_centroids.permute(0, 1, 3, 2))#type: ignore
...
@persistence.persistent_class
class MappingNetwork(torch.nn.Module):#type: ignore
    def __init__(self,
    ...
    def forward(self, z, c, pos = None, mask = None, truncation_psi = 1, truncation_cutoff = None, skip_w_avg_update = False):
        ...
        with torch.autograd.profiler.record_function("input"):#type: ignore
            ...
            z = normalize(z)
            x = g if self.transformer else z#type: ignore
        ...
        if self.w_avg_beta is not None and self.training and not skip_w_avg_update:
            with torch.autograd.profiler.record_function("update_w_avg"):#type: ignore
                self.w_avg.copy_(x.detach().mean(dim = (0, 1)).lerp(self.w_avg, self.w_avg_beta))

        # Broadcast
        if self.num_broadcast is not None:
            with torch.autograd.profiler.record_function("broadcast"):#type: ignore
                x = x.unsqueeze(2).repeat([1, 1, self.num_broadcast, 1])

        # Apply truncation
        if truncation_psi != 1:
            with torch.autograd.profiler.record_function("truncate"):#type: ignore
...
@persistence.persistent_class
class SynthesisLayer(torch.nn.Module):#type: ignore
    def __init__(self,
        in_channels,                    # Number of input channels
        out_channels,                   # Number of output channels
        ...
        **transformer_kwargs            # Arguments for SynthesisLayer
        ...
        if local_noise:
            noise_shape = list(misc.crop_tensor_shape((self.out_res, self.out_res), 
                transformer_kwargs.get("crop_ratio")))#type: ignore
            self.register_buffer("noise_const", torch.randn(noise_shape))#type: ignore
            self.noise_strength = torch.nn.Parameter(torch.zeros([]))#type: ignore
    ...
    def forward(self, x, y, att_vars = None, pos = None, mask = None, noise_mode = "random", fused_modconv = True):
        ...
        if self.transformer is not None:
            ...
            x, att_map, att_vars = self.transformer(
                ...
                att_vars = att_vars,        att_mask = mask.unsqueeze(1),#type: ignore
                hw_shape = shape[-2:]
            )
...
@persistence.persistent_class
class ToRGBLayer(torch.nn.Module):#type: ignore
...
@persistence.persistent_class
class SynthesisBlock(torch.nn.Module):#type: ignore
    def __init__(self,
        ...
        **layer_kwargs                      # Arguments for SynthesisLayer
        ...
        if self.stem:
            self.init_shape = misc.crop_tensor_shape((self.res, self.res), layer_kwargs.get("crop_ratio"))#type: ignore
            if latent_stem:
                ...
                self.conv_stem = FullyConnectedLayer(self.w_dim, out_channels * np.prod(self.init_shape), 
                    act = layer_kwargs.get(act, "lrelu"), gain = np.sqrt(2) / 4)           #type: ignore     
                self.num_conv += 1
            else:
                self.const = torch.nn.Parameter(torch.randn([out_channels, *self.init_shape]))#type: ignore
        else:
...
@persistence.persistent_class
class SynthesisNetwork(torch.nn.Module):#type: ignore
    def __init__(self,
        w_dim,                         # Intermediate latent (W) dimensionality
        k,                             # Number of latent vector components w_1,...,w_k
    ...
    def forward(self, ws, **block_kwargs):
        block_ws = []
        with torch.autograd.profiler.record_function("split_ws"):#type: ignore
...
@persistence.persistent_class
class Generator(torch.nn.Module):#type: ignore
    def __init__(self,
    ...
    def forward(self, z = None, c = None, ws = None, truncation_psi = 1, truncation_cutoff = None, return_img = True, 
        ...
        mask = random_dp_binary([_input.shape[0], self.k - 1], self.component_dropout, self.training, _input.device)#type: ignore
...
@persistence.persistent_class
class DiscriminatorBlock(torch.nn.Module):#type: ignore
    def __init__(self,
        in_channels,                        # Number of input channels
        out_channels,                       # Number of output channels
        resolution,                         # Resolution of this block
...
@persistence.persistent_class
class MinibatchStdLayer(torch.nn.Module):#type: ignore
...
@persistence.persistent_class
class DiscriminatorEpilogue(torch.nn.Module):#type: ignore
    def __init__(self,
        in_channels,                    # Number of input channels
...
@persistence.persistent_class
class Discriminator(torch.nn.Module):#type: ignore
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality
    ...
    def forward(self, img, c, **block_kwargs):
        img = misc.crop_tensor(img, self.crop_ratio)#type: ignore
        img_shape = misc.crop_tensor_shape((self.img_resolution, self.img_resolution), self.crop_ratio)#type: ignore
