...
def vis(G,
    ...
    for idx in crange(0, num, section_size):
        ...
        if attention:
            if "maps" in vis:
                ...
                maps = (soft_maps == np.amax(soft_maps, axis = 1, keepdims = True)).astype(float) # type: ignore
            ...
            if "layer_maps" in vis:
                ...
                for layer, lmaps in enumerate(maps_fakes):
                    ...
                    for head, hmap in enumerate(lmaps):
                        ...
                        hmap = np.sum(pallete * hmap, axis = 1) #type: ignore
                ...
                for maps, name in all_maps:
                    if grid:
                        ...
                        pattern = f"{run_dir}/visuals/layer_maps/%06d/{stepdir}/{name}.png" #type: ignore
    ...
    if "interpolations" in vis:
        ...
        for i in crange(rich_num):
            ...
            z = get_rnd_latents(2)
            z[0] = latents0[i] #type: ignore
            c = labels0[i:i+1] #type: ignore
            ...
            def save_interpolation(imgs, name):
                ...
                for c in range(k - 1):
                    ...
                    imgs[c] = [misc.to_pil(img, drange = drange_net) for img in imgs[c]] #type: ignore
    ...
    if "noise_var" in vis:
        ...
        imgs = np.stack([misc.to_pil(img, drange = drange_net) for img in imgs], axis = 0) #type: ignore
    ...
    if "style_mix" in vis:
        ...
        # Create latent mixes
        mixes = {
            "layer": (np.arange(wlatents_all_layers.shape[2]) < row_lens[:,None]).astype(np.float32)[:,None,None,None,:,None], #type: ignore
            "component": (np.arange(wlatents_all_layers.shape[1]) < row_lens[:,None]).astype(np.float32)[:,None,None,:,None,None] #type: ignore
        }
        ws = wlatents_all_layers[:cols+rows] #type: ignore
        orig_imgs = images[:cols+rows] #type: ignore
        col_z = wlatents_all_layers[:cols][None, None] #type: ignore
        row_z = wlatents_all_layers[cols:cols+rows][None,:,None] #type: ignore

        for name, mix in mixes.items():
            # Produce image mixes
            mix_z = mix * row_z  + (1 - mix) * col_z
            mix_z = torch.from_numpy(np.reshape(mix_z, [-1, *wlatents_all_layers.shape[1:]])).to(device) #type: ignore
            ...
            for row_idx, row_elem in enumerate([None] + list(range(len(row_lens) * rows))):
                for col_idx, col_elem in enumerate([None] + list(range(cols))):
                    ...
                    elif col_elem is None:                  img = orig_imgs[cols + (row_elem % rows)] #type: ignore
                    else:                                   img = mix_imgs[row_elem, col_elem] #type: ignore
