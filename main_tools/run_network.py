...
def setup_config(run_dir, **args):
    ...
    if not (num_gpus >= 1 and num_gpus & (num_gpus - 1) == 0): #type: ignore
        misc.error("Number of GPUs must be a power of two")
    args.num_gpus = num_gpus #type: ignore
    ...
    if args.components_num > 0:
        ...
        if args.latent_size % args.components_num != 0:
            misc.error(f"--latent-size ({args.latent_size}) should be divisible by --components-num (k={k})") #type: ignore
...
def subprocess_fn(rank, args, temp_dir):
    ...
    if args.num_gpus > 1:
        ...
        if os.name == "nt":
            ...
            torch.distributed.init_process_group(backend = "nccl", init_method = init_method, rank = rank, world_size = args.num_gpus) #type: ignore
...
def launch_processes(config):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        if config.num_gpus == 1:
            ...
            torch.multiprocessing.spawn(fn = subprocess_fn, args = (config, temp_dir), nprocs = config.num_gpus) #type: ignore
