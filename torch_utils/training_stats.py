# Facilities for reporting and collecting training statistics across multiple 
# processes and devices. The interface is designed to minimize synchronization 
# overhead as well as the amount of boilerplate in user code
...
# The averages are first collected into internal counters that are not directly 
# visible to the user. They are then copied to the user-visible state as a result 
# of calling `update()` and can then be queried using `mean()`, `std()`, `as_dict()`, 
# etc. Calling `update()` also resets the internal counters for the next round, 
# so that the user-visible state effectively reflects averages collected between 
...
def _sync(names):
    ...
    if _sync_device is not None:
        torch.distributed.all_reduce(deltas) #type: ignore
