...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, c, sync):
        with misc.ddp_sync(self.G, sync):
            ...
            if self.style_mixing > 0: 
                with torch.autograd.profiler.record_function("style_mixing"):#type: ignore
            ...
            if self.component_mixing > 0:
                with torch.autograd.profiler.record_function("component_mixing"): #type: ignore
    ...
    def accumulate_gradients(self, stage, real_img, real_c, gen_z, gen_c, sync, gain):
        ...
        if G_main:
            with torch.autograd.profiler.record_function("G_main_forward"): #type: ignore
                ...
                if self.g_loss == "logistic":
                    loss_G_main = -torch.nn.functional.softplus(gen_logits) # -log(sigmoid(gen_logits))#type: ignore
                ...
                training_stats.report("Loss/G/loss", loss_G_main)#type: ignore
            with torch.autograd.profiler.record_function("G_main_backward"):#type: ignore
                loss_G_main.mean().mul(gain).backward()#type: ignore

        # G_pl: Apply path length regularization
        if G_pl:
            with torch.autograd.profiler.record_function("G_pl_forward"):#type: ignore
                batch_size = gen_z.shape[0] // self.pl_batch_shrink
                gen_img, gen_ws = self.run_G(gen_z[:batch_size], gen_c[:batch_size], sync = sync)
                pl_noise = torch.randn_like(gen_img) / np.sqrt(gen_img.shape[2] * gen_img.shape[3])
                with torch.autograd.profiler.record_function("pl_grads"), conv2d_gradfix.no_weight_gradients():#type: ignore
                    pl_grads = torch.autograd.grad(outputs=[(gen_img * pl_noise).sum()], inputs=[gen_ws], create_graph = True, only_inputs = True)[0] #type: ignore
            ...
            with torch.autograd.profiler.record_function("G_pl_backward"):#type: ignore
                (gen_img[:, 0, 0, 0] * 0 + loss_G_pl).mean().mul(gain).backward()

        # D_main: Minimize logits for generated images
        loss_D_gen = 0
        if D_main:
            with torch.autograd.profiler.record_function("D_gen_forward"):#type: ignore
                ...
                if self.d_loss == "logistic":
                    loss_D_gen = torch.nn.functional.softplus(gen_logits) # -log(1 - sigmoid(gen_logits))#type: ignore
                elif self.d_loss == "hinge":
                    loss_D_gen = torch.clamp(1.0 + gen_logits, min = 0)
                elif self.d_loss == "wgan":
                    loss_D_gen = gen_logits

            with torch.autograd.profiler.record_function("D_gen_backward"):#type: ignore
                loss_D_gen.mean().mul(gain).backward()#type: ignore
        ...
        if D_main or D_r1:
            ...
            with torch.autograd.profiler.record_function(name + "_forward"):#type: ignore
                ...
                if D_main:
                    if self.d_loss == "logistic":
                        loss_D_real = torch.nn.functional.softplus(-real_logits) # -log(sigmoid(real_logits))#type: ignore
                    elif self.d_loss == "hinge":
                        loss_D_real = torch.clamp(1.0 - real_logits, min = 0)
                    elif self.d_loss == "wgan":
                        loss_D_real = -real_logits + tf.square(real_logits) * wgan_epsilon#type: ignore

                    training_stats.report("Loss/D/loss", loss_D_gen + loss_D_real)

                loss_D_r1 = 0
                if D_r1:
                    with torch.autograd.profiler.record_function("r1_grads"), conv2d_gradfix.no_weight_gradients(): #type: ignore
                        r1_grads = torch.autograd.grad(outputs=[real_logits.sum()], inputs=[real_img_tmp], create_graph = True, only_inputs = True)[0] #type: ignore
                    r1_penalty = r1_grads.square().sum([1,2,3])
                    loss_D_r1 = r1_penalty * (self.r1_gamma / 2)
                    training_stats.report("Loss/r1_penalty", r1_penalty)
                    training_stats.report("Loss/D/reg", loss_D_r1)
 
            with torch.autograd.profiler.record_function(name + "_backward"): #type: ignore
