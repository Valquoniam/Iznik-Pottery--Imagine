...
def load_dataset(dataset_args, batch_size, rank, num_gpus, log):
    ...
    dataset_iter = iter(torch.utils.data.DataLoader(dataset = dataset, sampler = dataset_sampler, #type: ignore 
...
def fetch_data(dataset, dataset_iter, input_shape, drange_net, device, batches_num, batch_size, batch_gpu):
    with torch.autograd.profiler.record_function("data_fetch"): #type: ignore
...
def distribute_nets(G, D, Gs, device, num_gpus, log):
    ...
    for name, net in [("G", G), ("D", D), (None, Gs)]: # ("G_mapping", G.mapping), ("G_synthesis", G.synthesis)
        if (num_gpus > 1) and (net is not None) and len(list(net.parameters())) != 0:
            ...
            net = torch.nn.parallel.DistributedDataParallel(net, device_ids = [device], broadcast_buffers = False, #type: ignore
...
def load_nets(load_pkl, nets, device, log):
    if (load_pkl is not None) and log:
        ...
        resume_data = loader.load_network(load_pkl)#type: ignore
...
def init_cuda(rank, cudnn_benchmark, allow_tf32):
    ...
    torch.backends.cudnn.benchmark = cudnn_benchmark    # Improves training speed#type: ignore
    torch.backends.cuda.matmul.allow_tf32 = allow_tf32  # Allow PyTorch to internally use tf32 for matmul#type: ignore
    torch.backends.cudnn.allow_tf32 = allow_tf32        # Allow PyTorch to internally use tf32 for convolutions#type: ignore
...
def setup_training_stages(loss_args, G, cG, D, cD, ddp_nets, device, log):
    ...
    for stage in stages:
        ...
        if log:
            stage.start_event = torch.cuda.Event(enable_timing = True)#type: ignore
            stage.end_event = torch.cuda.Event(enable_timing = True)#type: ignore
    return loss, stages
...
def run_training_stage(loss, stage, device, real_img, real_c, gen_z, gen_c, batch_size, batch_gpu, num_gpus):
    ...
    if stage.start_event is not None:
        stage.start_event.record(torch.cuda.current_stream(device))#type: ignore
    ...
    with torch.autograd.profiler.record_function(stage.name + "_opt"):#type: ignore
    ...
    if stage.end_event is not None:
        stage.end_event.record(torch.cuda.current_stream(device))#type: ignore
...
def update_ema_network(G, Gs, batch_size, cur_nimg, ema_kimg, ema_rampup):
    with torch.autograd.profiler.record_function("Gs"):#type: ignore
...
def snapshot_img_grid(Gs, drange_net, grid_z, grid_c, grid_size, batch_gpu, truncation_psi, suffix = "init"):
    ...
    misc.save_img_grid(images, os.path.join(run_dir, f"fakes_{suffix}.png"), drange = drange_net, grid_size = grid_size)#type: ignore
...
def init_logger(run_dir, log):
    ...
    if log:
        ...
        try:
            ...
            logger.tfevents = tensorboard.SummaryWriter(run_dir)#type: ignore
...
def training_loop(
    ...
    G, D, Gs = load_nets(resume_pkl, nets, device, log)         #type: ignore                  # Resume from existing pickle
    ...
    while True:
        ...
        if stats is not None:
            ...
            fields.append(f"CPU {training_stats.report0('Resources/peak_gpu_mem_gb', torch.cuda.max_memory_allocated(device) / 2**30):<6.2f}")#type: ignore
