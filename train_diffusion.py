...
from models import ddpm, orig_unet
...
class GenModel(pl.LightningModule):
    def __init__(self, data, size, in_c, n_steps, lr, device='cuda:0'):
        super().__init__()
        self.data = data
        self.size = size
        self.in_c = in_c
    ...
    def forward(self, x, noise):
        ...
        noise_pred = self.ddpm.reverse(noisy, timesteps, None, None, None, None)
        return noise_pred
...
if __name__ == '__main__':
    ...
    model = GenModel(data='tiles', size=size, in_c=c, n_steps=timesteps, lr=learning_rate,
                     device=device)

    exp_dir = f'temp_tiles_unet_size_{size}_steps_{timesteps}_lr_{learning_rate}_aug_{",".join(augment_modes)}'
