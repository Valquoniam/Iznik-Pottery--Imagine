...
def parse_args_and_config():
    ...
    # Text
    parser.add_argument('--edit_attr', type=str, default=None, help='Attribute to edit defiend in ./utils_asyrp/text_dic.py')
    ...
    parser.add_argument('--hybrid_noise', type=int, default=0, help='Whether to change multiple attributes by mixing multiple models_asyrp')
    ...
    os.makedirs('./logs_asyrp', exist_ok=True)
    os.makedirs(args.exp, exist_ok=True)

    sh_file_name = os.path.join('./asyrp_scripts', args.sh_file_name)

    import shutil
    if args.run_test:
        shutil.copy(sh_file_name, os.path.join(args.exp, f"{(args.sh_file_name).split('.')[0]}_test.sh"))
    elif args.style_transfer:
        shutil.copy(sh_file_name, os.path.join(args.exp, f"{(args.sh_file_name).split('.')[0]}_style_transfer.sh"))
    elif args.run_train:
        shutil.copy(sh_file_name, os.path.join(args.exp, f"{args.sh_file_name.split('.')[0]}_train.sh"))
    elif args.lpips:
        pass
